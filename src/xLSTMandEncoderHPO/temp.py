...
def augmentWithTimeReversal(X_train, y_train):
    ...
    ECG signals enter this function as (samples, timesteps).  Only axis 1 is
    reversed, so sample order and labels are unchanged.  copy() removes the
    negative NumPy stride produced by slicing, which PyTorch tensors do not
    support.
    """
    if X_train.ndim != 2:
...
def crossValidatedScore(params, X_development, y_development, folds, device, trial):
    ...
    for fold_number, (train_index, val_index) in enumerate(folds, start=1):
        ...
        # Augment only the training side of this fold.  Reversing after the
        # split prevents a forward signal and its reversed copy from landing
        # on opposite sides of the train/validation boundary.
        X_train, y_train = augmentWithTimeReversal(X_train, y_train)
...
def runCrossValidatedModelSelection(task, model_type, output_prefix, roc_suffix):
    ...
    # Add exactly one reversed copy of each final-training observation. 
    X_train_augmented, y_train_augmented = augmentWithTimeReversal(
        X_train_scaled, y_train
    )

    # Duplicate minority-class rows in the training set only.
    ros = RandomOverSampler(random_state=RANDOM_STATE)
    X_train_ros, y_train_ros = ros.fit_resample(
        X_train_augmented, y_train_augmented
    )
    print(
        "Training class counts original/reversed augmentation/oversampling:",
        np.bincount(y_train),
        "->",
